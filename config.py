...
@dataclass
class Config:
    mode: str = os.getenv("MODE", "backtest")  # "live" or "backtest"
    ...
    order_size: float = float(os.getenv("ORDER_SIZE", 0.001))  # Min for BTCUSDT
    ...
    refresh_interval: float = float(os.getenv("REFRESH_INTERVAL", 5.0))  # seconds
    max_drawdown: float = float(os.getenv("MAX_DRAWDOWN", 20.0))  # USDT
    # Backtest params
